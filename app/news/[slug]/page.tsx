...
export default function NewsDetailsPage() {
  ...
  return newsData !== undefined ? (
    <div className="w-[90%] mx-auto flex flex-col gap-y-5 mt-5">
      <div className="w-full flex items-center gap-x-5">
        <div className="flex flex-col items-start gap-y-3 w-[40%]">
          <div className="text-blue-600 font-thin text-xl rounded-xl px-4 py-2 border border-blue-600">
            {newsData.cat}
          </div>
          <h1 className="font-bold text-5xl leading-[65px]">
            {newsData.title}
          </h1>
          <h2 className="font-thin text-neutral-800 text-xl leading-[35px]">
          ...
          <div className="flex items-center gap-x-2">
            <div
              ...
              className="rounded-full w-12 h-12 border border-neutral-300 mt-2"
            ></div>
            <section className="flex flex-col gap-y-1">
              <h1 className="font-thin text-neutral-800 text-sm">
                نوشته شده توسط حمیدرضا قناعتیان
              </h1>
              <h1 className="font-thin text-neutral-800 text-sm">
                تاریخ انتشار: {convertToFarsiNumbers(newsData.date.toString())}
              </h1>
            </section>
          </div>
        </div>
      ...
      <p className="mt-3 text-lg font-thin leading-[35px]">{newsData.details}</p>
    </div>
  ) : (
    <h1 className="text-center font-bold">در حال دریافت اطلاعات مقاله</h1>
  );
}
